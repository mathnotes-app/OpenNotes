...
async function recordSignal(signal: ReviewSignal): Promise<void> {
...
export function recordReviewSignal(signal: ReviewSignal): Promise<void> {
  return stateQueue.enqueue(() => recordSignal(signal)).catch((error) => {
    if (__DEV__) console.warn('[reviewPromptService] signal failed', error);
  });
}
